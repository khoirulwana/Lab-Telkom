...
export default {
  ...
  setup() {
    ...
    // Update charts for AM
    const updateCharts = () => {
      if (!inputSignalChart || !outputSignalChart || !frequencyChart) return;
      // Sinyal informasi, pembawa, dan AM (animated)
      const animate = (timestamp) => {
        if (!startTime) startTime = timestamp;
        const elapsed = timestamp - startTime;
        const timePoints = Array.from({ length: 100 }, (_, i) => i / 100);
        // Phase for smooth animation
        const phaseInfo = (elapsed / 2000) * signalFreq.value * 2 * Math.PI;
        const phaseCarrier = (elapsed / 2000) * carrierFreq.value * 2 * Math.PI;
        // Info signal
        const infoData = timePoints.map(
          (t) =>
            signalAmp.value *
            Math.sin(2 * Math.PI * signalFreq.value * t + phaseInfo)
        );
        // Carrier signal
        const carrierData = timePoints.map(
          (t) =>
            carrierAmp.value *
            Math.sin(2 * Math.PI * carrierFreq.value * t + phaseCarrier)
        );
        // AM signal
        const amData = timePoints.map(
          (t) =>
            (carrierAmp.value +
              signalAmp.value *
                Math.sin(2 * Math.PI * signalFreq.value * t + phaseInfo)) *
        ...
        frequencyChart.data.datasets[0].data = amData;
    ...
    watch([signalFreq, signalAmp, carrierFreq, carrierAmp], () => {
      updateCharts();
    });
